...
def ImageRecognition():
    ...
    recognizer_mask = cv.face.LBPHFaceRecognizer_create()
    recognizer_mask.read("StudentLabel\Mask Trainner.yml")
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            mask_color = (0, 0, 0)
            sid, conf_mask = recognizer_mask.predict(roi_gray)
            ...
            if (len(mouth) == 0 and len(nose) == 0):
                ...
                if 20 <= conf_mask <= 65:
                    ...
                    # confstr = "  {0}%".format(round(120 - conf1))
            ...
            cv.putText(frame, status, (x, y + h + 90),
                       font, 0.5, mask_color, 2)
            # cv.putText(frame, str(confstr), (x + 5, y + h - 5),
            #            font, 0.5, mask_color, 2)
